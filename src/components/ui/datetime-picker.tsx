...
function Calendar({
  ...
  const disableLeftNavigation = () => {
    ...
    if (props.month) {
      const month = new Date(props.month);
      return (
...
const TimePeriodSelect = React.forwardRef<HTMLButtonElement, PeriodSelectorProps>(
  ({ period, setPeriod, onDateChange, onLeftFocus, onRightFocus, ...props }, ref) => {
...
const TimePickerInput = React.forwardRef<HTMLInputElement, TimePickerInputProps>(
  (
    ...
    const date = React.useMemo(
      () => (props.date ? new Date(props.date) : new Date(new Date().setHours(0, 0, 0, 0))),
      [props.date],
    );
...
const TimePicker = React.forwardRef<TimePickerRef, TimePickerProps>(
  ({ onChange, hourCycle = 24, granularity = 'second', ...props }, ref) => {
    const date = props.date ? new Date(props.date) : undefined;
...
type DatetimePickerProps = {
   ...
   */
  minValue?: Date | string | undefined | null;
  /**
   * The latest date that the user can select
   */
  maxValue?: Date | string | undefined | null;
...
const DatetimePicker = React.forwardRef<Partial<DatetimePickerRef>, DatetimePickerProps>(
  (
    ...
    const minValue = props.minValue ? new Date(props.minValue) : undefined;
    const maxValue = props.maxValue ? new Date(props.maxValue) : undefined;
    const defaultPopupValue = React.useMemo(
      () =>
        props.defaultPopupValue
          ? new Date(props.defaultPopupValue)
          : (value ?? new Date(new Date().setHours(0, 0, 0, 0))),
      [props.defaultPopupValue, value],
    );
    ...
    /**
     * react to external changes in the value prop
     */
    React.useEffect(() => {
    ...
    const handleSelect = (newDay: Date | undefined) => {
      ...
      if (minValue && newDay < minValue) {
        onChange?.(minValue);
        setMonth(minValue);
        return;
      }
      if (maxValue && newDay > maxValue) {
        onChange?.(maxValue);
        setMonth(maxValue);
        return;
      }
        ...
        <PopoverContent className='w-auto p-0'>
          <Calendar
            ...
            disabled={(date) => {
              ...
              const min = minValue && startOfDay(minValue);
              const max = maxValue && startOfDay(maxValue);
